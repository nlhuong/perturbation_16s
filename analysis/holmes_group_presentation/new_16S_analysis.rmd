...
meas_info <- meas %>%
  left_join(samp) %>%
  left_join(subj_data) 
write.csv(meas_info, file.path(datadir, "sample_info/Mapping_File_19Oct2018.xlsx"))
```

```{r, eval = FALSE}
meas_info <- read.csv( file.path(datadir, "sample_info/Mapping_File_19Oct2018.xlsx"), row.names = 1) %>%
...
```{r eval = FALSE}
saveRDS(ps, file = file.path(datadir, "16S/phyloseq/perturb_physeq_fltr2430_17Jul18.rds"))
```

...
```{r distr-seq-samples, fig.width=10, fig.height=9}

df2plot <- data.frame(sample_data(ps)) %>%
  select(Meas_ID, Samp_ID, Subject, Samp_Date, Meas_Type, 
         Group, Interval) %>%
  mutate(Samp_Date = as.Date(Samp_Date)) %>%
  filter(!grepl("NA", Subject))
...
familyfactor <- factor(as(tax_table(ps.pa.pos), "matrix")[, "Family"], levels = topFamily)
ggplot(data=df.pa, aes(x=pa.neg, y=pa.pos, color=familyfactor)) + geom_point() +
  xlab("Prevalence (Negative Controls)") + ylab("Prevalence (Subject Samples)") +
...
```{r}
cross_contam_seqs <- df.pa %>% rownames_to_column("SeqID") %>% filter(pa.neg > 25) %>% .[["SeqID"]]
...
```{r, fig.width=8, fig.height=6}
familyfactor <- factor(as(tax_table(ps.pa.pos), "matrix")[, "Family"], levels = topFamily)
ggplot(data=df.pa.mid, aes(x=pa.neg, y=pa.pos, color=familyfactor)) + geom_point() +
...
```{r}
cross_contam_seqs_mid_perturb <- df.pa.mid %>% rownames_to_column("SeqID") %>% 
